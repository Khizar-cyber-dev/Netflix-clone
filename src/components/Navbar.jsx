...
const Navbar = () => {
  ...
  return (
    ...
    <div className='navbar-right flex gap-[20px] items-center'>
        ...
        <div className="navbar-profile flex items-center gap-[10px] cursor-pointer relative" onMouseEnter={()=> setHover(true)} >
            <img src={profile_img} alt="profile_icon" className='profile rounded w-[35px]' />
            <img src={caret_icon} alt="caret_icon" /> {hover && (
            <div className="dropdown absolute top-10 right-0 w-max bg-[#191919] py-[18px] px-[22px] rounded z-10" onMouseLeave={()=> setHover(false)}>
                <p className='text-sm cursor-pointer hover:underline' onClick={()=>{navigate('/')}}>Sign Out of Netflix</p>
            </div>
            )}
        </div>
    </div>
